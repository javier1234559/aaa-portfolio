...
- **Source of truth for colors, type, and rhythm:** `src/app/globals.css` (`:root`, `.dark`, `@theme inline`). Align new UI with these tokens before introducing one-off hex values.
- **Reference prototype:** static Vite mock under `design/aaa-portfolio/` (spacing and layout ideas only — the production app is Next.js in `src/`).
- **Brand copy + favicon workflow:** `docs/BRAND.md`.
- **Principles:** 8px spacing rhythm (Tailwind `gap-2` = 8px, prefer multiples of 2); cards and shells favor **16–24px** radius (`rounded-2xl` / `rounded-3xl`); controls often **pill** (`rounded-full`). **Colors & type:** see `docs/BRAND.md` (blue primary, cool canvas, Inter UI font). Enterprise SaaS density — subtle borders (`border-border/70`), restrained `shadow-sm`, compact body copy (`text-sm` in tables/dense panels).
