...
**This file mirrors** `idea/.cursor/skills/aaa-portfolio-config/SKILL.md`. Keep both in sync when changing workflow steps.
...
2. **Folder** — Ensure `idea/config/projects/<slug>/` exists with:
   - `<slug>.yml` (required): align with `aaa-client-dashboard/app/src/app/config/clients.ts`; set `clientDashboardPath`, `jiraProjectKey`, and `githubRepo` from dashboard conventions and `.github/workflows/sync-*.yml`.

3. **JSON from automation** — `data_sprint_progress.json` / `data_github_activity.json` are updated by **monorepo root** `.github/workflows/portfolio-sync-*.yml` (see `idea/config/ci-sync-manifest.json`). You can still copy from `aaa-client-dashboard` if you prefer a single sync source.
...
6. **Verify** — `cd idea && npm run build`.

## Do not

- Commit secrets into config files.
- Rely on writing to git-tracked JSON from Vercel serverless routes without an explicit GitHub API or external store (see `docs/PLANS.md` §7.2 and §7.6).
